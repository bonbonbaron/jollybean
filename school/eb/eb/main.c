        ...
        if ( parentPropertyNodeP->type == XML_ATTRIBUTE_NODE ) {
          ...
          if ( !(strcmp( (char*) parentPropertyNodeP->name, "id" ) ) ) {
            ...
            if ( propChildP && propChildP->type == XML_TEXT_NODE ) {
              ...
              // TRIANGLES
              // =========
              else if( strstr( (char*) propChildP->content, "mesh-vertices" ) > 0 ) {
                ...
                // forEachProperty_( topNodePP, curr ) {
...
void pack(U16* array, int bits, U8** result) {
  ...
  arrayNew( (void**) result, sizeof( U8 ), ( nInputElems * bits + 7 ) / 8); // i have no clue why it has to be this
...
int main ( int argc, char **argv ) {
  ...
  */
  xmlChar* GEOMETRY_DATA_XPATH = ( xmlChar * ) "/mb:COLLADA/mb:library_geometries/mb:geometry";
  xmlChar* VERTEX_DATA_XPATH = ( xmlChar * ) "/mb:mesh/mb:source/mb:float_array";
  xmlChar* TRIANGLE_DATA_XPATH = ( xmlChar * ) "/mb:mesh/mb:triangles";
  ...
  xmlXPathContextPtr nsContext = xmlGetNamespaceContext( docP, NAMESPACE_ALIAS );
  if ( nsContext == NULL ) {
    printf( "null nsContext\n" );
    return 1;
  }

  // Execute XPath searches.
  xmlXPathObjectPtr vertexXpathResult   = xmlGetNodes ( docP, nsContext, VERTEX_DATA_XPATH );
  xmlXPathObjectPtr triangleXpathResult = xmlGetNodes ( docP, nsContext, TRIANGLE_DATA_XPATH );
  ...
  assert( qPosA );
  Vec3 range;
  range.x = fabs( mesh.posResult.max.position.x - mesh.posResult.min.position.x );
  range.y = fabs( mesh.posResult.max.position.y - mesh.posResult.min.position.y );
  range.z = fabs( mesh.posResult.max.position.z - mesh.posResult.min.position.z );
  const float convX = 1024.0 / range.x;
  const float convY = 1024.0 / range.y;
  const float convZ = 1024.0 / range.z;

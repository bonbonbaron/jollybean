...
Error init(Database *db_p, Uint8 *bp_p) {
  Error stat = 0;
  ...
  /* Scene Zero */
  if (!stat) {
    scene = &db_p->tblScene[0];  /* First index is always the scene the game starts with. */
  }

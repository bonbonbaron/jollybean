...
typedef struct {
  ...
  Map      *conditionMP;  // This maps node indices to the U8 condition flags specifically enumerated for the condition node  // TODO make this a switchU instead.
} BTreeStatus;

typedef NodeStat (*NodeCb)(struct Node *rootP, struct Node *currNodeP, BTreeStatus *bTStatP, Map *bbMP, Message *outboxF);  
...
#define LeafNodeFuncDef_(name_, cb_, ...)\
  SrcNodeDef_(name_, cb_, __VA_ARGS__)

#define SelectorNodeFuncDef_(name_, ...)\
  SrcNodeDef_(name_, btSelector, __VA_ARGS__)

#define SequenceNodeFuncDef_(name_, ...)\
  SrcNodeDef_(name_, btSequence, __VA_ARGS__)

#define SrcNodeDef_(name_, cb_, ...)\
...
#define NodeFuncDef_(name_) NodeStat name_(Node *rootP, Node *currNodeP, BTreeStatus *btStatP, Map *bbMP, Message *outboxF) 
#define nodeRun_ _nodeRun(rootP, currNodeP, btStatP, bbMP, outboxF)
...
NodeStat btRun(BTree *treeP, BTreeStatus *btStatP, Map *bbMP, Message *outboxF);  

...
#include <stdio.h>
#include <stdlib.h>
#include "../libbotox/src/include/data.h"
static FILE *fP;
...
void fluid_set_default_fileapi (fluid_fileapi_t * fileapi) {
	...
	fluid_default_fileapi =
		fileapi == NULL ? (fluid_fileapi_t *) & default_fileapi : fileapi;
}
...
fluid_sfont_t *fluid_defsfloader_load (fluid_sfloader_t * loader, const char *filename) {
  ...
  // MB: Soundfont file-writing starts here.
  fP = fopen("./sf3Out.c", "w");
  ...
  // MB: This'll write the damn soundfonts in my extremely awkward SF3 writer. lol
  //     1) Keep the samples in OGG format. This entails ultra-awkward file I/O in
  //        the many different SF-loading functions of this document. 
  //     2) Write each piece of the soundfont separately. Yes, this will be painful.

  fluid_list_t *nodeP = defsfont->sample;
...
fluid_preset_t *fluid_defsfont_sfont_get_preset (fluid_sfont_t * sfont,
	...
	defpreset =
		fluid_defsfont_get_preset ((fluid_defsfont_t *) sfont->data, bank,
															 prenum);
...
int fluid_defpreset_preset_noteon (fluid_preset_t * preset,
 ...
 */
int fluid_defsfont_add_sample (fluid_defsfont_t * sfont,
															 fluid_sample_t * sample) {
...
fluid_defsfont_load_sampledata (fluid_defsfont_t * sfont,
  ...
  // MB: Write this sample (OGG?) to the output file.
  typedef long unsigned int UL32;
  Inflatable inf;
  UL32 sizeDataOriginal = sfont->samplesize * 2; // try this since casting U16* to U8*
  UL32 sizeDataCompressed  = (sizeDataOriginal * 1.1) + 12;
  S8* dataCompressed = (S8*)malloc( sizeDataCompressed );

  // Now hold on, you ask.  WHY is the array // that's supposed to hold the COMPRESSED data ALREADY BIGGER than the original data array?  This isn't compression!  This is meaningless expansion!
  // Well, you'll see that this extra space in the compressed array is only TEMPORARY.  Just suffice it to say that zlib "needs room to breathe".
  // When zlib performs compression, it will need a bit of extra room to do its work.
  // When the compress() routine returns, the compressedData array will have been AUTOMATICALLY RESIZED by ZLIB to being a smaller, compressed size.
  // We will also know the EXACT size of that compressed data by looking at the 'sizeDataCompressed' variable AFTER the compress() routine runs.  That variable 'sizeDataCompressed' will updated by the compress() function when we call it!
  // Don't worry, the "compressed" data will be smaller than the original data was in the end!
  //
  U8 *dataOriginal = (U8*) sfont->sampledata;
  ...
  printf("* SOUNDFONT COMPRESSION COMPLETE!! :\n");

  int counter = 0;
  fprintf(fP, "// OGG sample data compressed again through zlib\n");
  fprintf(fP, "Inflatable sf3Inf = {\n");
  fprintf(fP, "\t.compressedLen  = %d,\n", sizeDataCompressed);;
  fprintf(fP, "\t.inflatedDataP  = NULL,\n");
  fprintf(fP, "\t.inflatedLen    = %d,\n", sizeDataOriginal);;
  fprintf(fP, "\t.compressedData = {\n\t");
  U8 *charP = (U8*) dataCompressed;
  U8 *endCharP = charP + sizeDataCompressed;
  for (U32 lineElemCounter = 0; charP < endCharP; ++charP) {
    fprintf(fP, "0x%02x, ", *charP);
    if (++lineElemCounter >= 16) {
      fprintf(fP, "\n\t");
      lineElemCounter = 0;
    }
  }
...
#if 0
  fprintf(fP, "Inflatable sampleInf = {\n\t", counter++);
  S16 *sampleByteP = sfont->sampledata;
  for (int i = 0, j = 0; i < sfont->samplesize; ++i, ++j) {
    fprintf(fP, "0x%02x, ", *(sampleByteP++));
    if (j >= 15) {
      fprintf(fP, "\n\t");
      j = 0;
    }
...
static int process_info (int size, SFData * sf, void *fd,
												 fluid_fileapi_t * fapi);
static int process_sdta (int size, SFData * sf, void *fd,
												 fluid_fileapi_t * fapi);
static int pdtahelper (unsigned int expid, unsigned int reclen,
											 SFChunk * chunk, int *size, void *fd,
											 fluid_fileapi_t * fapi);
static int process_pdta (int size, SFData * sf, void *fd,
												 fluid_fileapi_t * fapi);
static int load_phdr (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_pbag (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_pmod (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_pgen (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_ihdr (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_ibag (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_imod (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
static int load_igen (int size, SFData * sf, void *fd,
											fluid_fileapi_t * fapi);
...
process_info (int size, SFData * sf, void *fd, fluid_fileapi_t * fapi) {
	...
	while (size > 0) {
		...
		if (id == IFIL_ID) {				/* sound font version chunk? */
			...
			if (chunk.size != 4)
				return (gerr (ErrCorr, _("ROM version info chunk has invalid size")));

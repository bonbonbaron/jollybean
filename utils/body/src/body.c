...
int main(int argc, char ** argv) {
  ...
  for (U32 i = 1; !e && i < argc; ++i) {
    ...
    e = getSrcFilePath(&srcFilePath, "Body/Graybody/Colormap", argv[i], ".png", verbose); 
    if (!e) {
    ...
    if (!e) {
      e = getSrcFilePath(&srcFilePath, "Body/Graybody/Animation", argv[i], ".json", verbose); 
    }
    if (!e) {
    ...
    // Collision
    if (!e) {
      e = getSrcFilePath(&srcFilePath, "Body/Graybody/Collision", argv[i], "_col.png", verbose); 
    }
    if (!e) {

...
int main(int argc, char ** argv) {
  ...
  for (U32 i = 1; !e && i < argc; ++i) {
    ...
    e = getSrcFilePath(&srcFilePath, "Body/Graybody/Colormap", ".png", argv[i]); 
    if (!e) {
    ...
    if (!e) {
      e = getSrcFilePath(&srcFilePath, "Body/Graybody/Animation", ".json", argv[i]); 
    }
    if (!e) {
    ...
    // Collision
    if (!e) {
      e = getSrcFilePath(&srcFilePath, "Body/Graybody/Collision", "_col.png", argv[i]); 
    }
    if (!e) {
